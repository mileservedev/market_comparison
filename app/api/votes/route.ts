...
function visitorKey(fingerprint: string) {
  const secret = process.env.VOTE_HASH_SECRET;
  if (!secret || secret.length < 32) {
    throw new Error(
      'VOTE_HASH_SECRET must be configured with at least 32 characters.',
    );
  }
  return createHmac('sha256', secret)
    .update(`pickwise-v1|${fingerprint}`)
    .digest('hex');
}

function safeDeviceDetails(value: unknown) {
  if (!value || typeof value !== 'object') return {};
  const input = value as Record<string, unknown>;
  return Object.fromEntries(
    ['language', 'platform', 'viewport', 'timezone']
      .filter((key) => typeof input[key] === 'string')
      .map((key) => [key, String(input[key]).slice(0, 128)]),
  );
}
...
export async function GET(request: Request) {
  try {
    ...
    const key = fingerprint ? visitorKey(fingerprint.slice(0, 128)) : undefined;
...
export async function POST(request: Request) {
  try {
    ...
    await pruneExpiredVotes();
    const db = getMySqlPool();
    const key = visitorKey(body.fingerprint.slice(0, 128));
    const userAgent = (request.headers.get('user-agent') || 'unknown').slice(
      0,
      512,
    );
    await db.execute<ResultSetHeader>(
      `INSERT IGNORE INTO votes (id, campaign, feature, product, visitor_key, user_agent, device_details, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
      [
        randomUUID(),
        campaignSlug,
        body.feature,
        ...
        key,
        userAgent,
        JSON.stringify(safeDeviceDetails(body.device)),
      ],
    );
