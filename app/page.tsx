...
import { Check, LockKeyhole, RefreshCw, ShieldCheck, Sparkles, ThumbsUp } from 'lucide-react';

type ProductId = 'stride' | 'velocity';
type FeatureId = 'comfort' | 'fit' | 'style' | 'color';
type VoteTotals = Record<FeatureId, Record<ProductId, number>>;

const products = {
  stride: { name: 'Stride X', edition: 'Midnight / Amber', image: '/stride-x.png', accent: '#f4b740' },
  velocity: { name: 'Velocity Pro', edition: 'Pearl / Signal', image: '/velocity-pro.png', accent: '#ff5147' },
} as const;

const features: { id: FeatureId; name: string; hint: string; x: string; y: string }[] = [
  { id: 'comfort', name: 'Comfort', hint: 'Cushioning & ride', x: '48%', y: '68%' },
  { id: 'fit', name: 'Fit', hint: 'Lockdown & sizing', x: '51%', y: '37%' },
  { id: 'style', name: 'Style', hint: 'Form & silhouette', x: '72%', y: '49%' },
  { id: 'color', name: 'Color', hint: 'Palette & finish', x: '29%', y: '50%' },
];

const emptyTotals: VoteTotals = { comfort: { stride: 0, velocity: 0 }, fit: { stride: 0, velocity: 0 }, style: { stride: 0, velocity: 0 }, color: { stride: 0, velocity: 0 } };

async function deviceFingerprint() {
  const raw = [navigator.userAgent, navigator.language, navigator.platform, `${screen.width}x${screen.height}x${screen.colorDepth}`, Intl.DateTimeFormat().resolvedOptions().timeZone, navigator.hardwareConcurrency ?? '', navigator.maxTouchPoints ?? ''].join('|');
...
export default function Home() {
  ...
  async function vote(feature: FeatureId, product: ProductId) {
    if (!fingerprint || busy) return; setBusy(feature); setMessage('');
    try {
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <section className="mx-auto max-w-[1480px] px-5 pb-16 pt-8 sm:px-8">
      ...
      <div className="grid gap-4 lg:grid-cols-2">{(Object.keys(products) as ProductId[]).map((productId, index) => { const product = products[productId]; return <article key={productId} className="product-card overflow-hidden rounded-[28px] border border-white/10 bg-[#111113] shadow-2xl shadow-black/20"><div className="flex items-center justify-between px-5 pt-5 sm:px-7"><div><p className="text-[10px] font-semibold uppercase tracking-[.2em] text-zinc-500">Contender 0{index + 1}</p><h2 className="mt-1 text-2xl font-semibold">{product.name}</h2><p className="text-xs text-zinc-500">{product.edition}</p></div><span className="rounded-full border border-white/10 px-3 py-1.5 text-[10px] font-medium uppercase tracking-[.15em] text-zinc-400">4 attributes</span></div><div className="relative mt-2 aspect-[16/10] overflow-hidden"><img src={product.image} alt={`${product.name} running shoe`} className="h-full w-full object-cover"/><div className="absolute inset-0 bg-gradient-to-t from-[#111113] via-transparent to-transparent"/>{features.map((feature) => { const selected = myVotes[feature.id] === productId; const locked = Boolean(myVotes[feature.id] && !selected); return <button key={feature.id} onClick={() => vote(feature.id, productId)} disabled={busy === feature.id} aria-label={`Vote for ${product.name} on ${feature.name}`} className={`feature-pin group absolute -translate-x-1/2 -translate-y-1/2 rounded-full border px-3 py-2 text-left transition duration-200 ${selected ? 'selected border-white bg-white text-black' : locked ? 'border-white/10 bg-black/65 text-zinc-500' : 'border-white/25 bg-black/75 text-white hover:-translate-y-[56%] hover:border-white/60'}`} style={{ left: feature.x, top: feature.y }}><span className="flex items-center gap-2"><span className="grid size-6 place-items-center rounded-full" style={{ background: selected ? product.accent : 'rgba(255,255,255,.1)' }}>{selected ? <Check size={13}/> : locked ? <LockKeyhole size={11}/> : <ThumbsUp size={12}/>}</span><span><strong className="block text-xs leading-none">{feature.name}</strong><small className="mt-1 hidden text-[9px] opacity-60 sm:block">{selected ? 'Your pick' : locked ? 'Picked opposite' : 'Tap to vote'}</small></span></span></button>; })}</div></article>; })}</div>
      <section className="mt-5 rounded-[28px] border border-white/10 bg-[#111113] p-5 sm:p-7"><div className="mb-6 flex items-start justify-between gap-4"><div><p className="text-[10px] font-semibold uppercase tracking-[.2em] text-zinc-500">Community report</p><h2 className="mt-1 text-xl font-semibold">Feature-by-feature results</h2></div><div className="flex items-center gap-2 text-xs text-zinc-500"><ShieldCheck size={15} className="text-emerald-400"/>One device, one choice per feature</div></div><div className="grid gap-x-8 gap-y-6 md:grid-cols-2">{features.map((feature) => { const pair = totals[feature.id]; const sum = pair.stride + pair.velocity; const left = sum ? Math.round(pair.stride / sum * 100) : 50; const right = 100 - left; return <div key={feature.id}><div className="mb-2 flex items-end justify-between"><div><p className="font-medium">{feature.name}</p><p className="text-xs text-zinc-500">{feature.hint}</p></div><p className="text-xs text-zinc-500">{sum} vote{sum === 1 ? '' : 's'}</p></div><div className="flex h-2 overflow-hidden rounded-full bg-zinc-800"><span className="bg-amber-400 transition-all" style={{ width: `${left}%` }}/><span className="bg-red-500 transition-all" style={{ width: `${right}%` }}/></div><div className="mt-2 flex justify-between text-xs"><span className="text-amber-300">Stride X <b>{left}%</b></span><span className="text-red-300"><b>{right}%</b> Velocity Pro</span></div></div>; })}</div></section>
