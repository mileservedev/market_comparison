...
import { Crown, RefreshCw, ShieldCheck, Sparkles, ThumbsUp } from 'lucide-react';
...
export default function Home() {
  ...
  const productVotes = useMemo(() => Object.values(totals).reduce((sum, votes) => ({ stride: sum.stride + votes.stride, velocity: sum.velocity + votes.velocity }), { stride: 0, velocity: 0 }), [totals]);
  const leader: ProductId | 'tie' | null = totalVotes === 0 ? null : productVotes.stride === productVotes.velocity ? 'tie' : productVotes.stride > productVotes.velocity ? 'stride' : 'velocity';
  const leadBy = Math.abs(productVotes.stride - productVotes.velocity);
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <section className="mx-auto max-w-[1480px] px-5 pb-16 pt-8 sm:px-8">
      <div className="mb-7 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="mb-2 text-xs font-semibold uppercase tracking-[.22em] text-amber-400">Running shoe showdown</p><h1 className="max-w-3xl text-3xl font-semibold tracking-tight sm:text-5xl">Every feature. One clear winner.</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-400 sm:text-base">Tap a feature on either shoe to cast your preference. Your first choice for each feature is final.</p></div><div className="flex items-center gap-3 rounded-2xl border border-white/8 bg-white/[.035] px-4 py-3"><div className="text-right"><p className="text-2xl font-semibold tabular-nums">{totalVotes}</p><p className="text-[10px] uppercase tracking-[.17em] text-zinc-500">Votes counted</p></div><RefreshCw className="text-zinc-600" size={18}/></div></div>
      <div className={`leader-banner mb-4 ${leader === 'velocity' ? 'nike-leading' : ''}`} role="status" aria-live="polite"><span className="leader-crown"><Crown size={26}/></span><div>{leader === null ? <><p className="leader-eyebrow">The crown is waiting</p><p className="leader-name">Be the first to vote</p></> : leader === 'tie' ? <><p className="leader-eyebrow">Neck and neck</p><p className="leader-name">adidas and Nike are tied</p></> : <><p className="leader-eyebrow">Current overall winner</p><p className="leader-name">{products[leader].name} is leading</p><p className="leader-detail">{productVotes[leader]} total votes · ahead by {leadBy}</p></>}</div></div>
      <div className="grid gap-4 lg:grid-cols-2">{(Object.keys(products) as ProductId[]).map((productId) => { const product = products[productId]; const winning = leader === productId; return <article key={productId} className={`product-card product-${productId} ${winning ? 'overall-winner' : ''} overflow-hidden rounded-[28px] border border-white/10 bg-[#111113] shadow-2xl shadow-black/20`}><div className="relative px-5 pt-6 text-center sm:px-7"><h2 className="text-2xl font-semibold uppercase tracking-[.12em]">{product.name}</h2>{winning && <span className="card-crown" aria-label="Current overall winner"><Crown size={18}/> Leading</span>}</div><div className="shoe-map relative mt-1 aspect-[3/2] overflow-hidden"><img src={product.image} alt={`${product.name} sectioned running shoe diagram`} className={`pointer-events-none h-full w-full object-contain opacity-80 ${productId === 'velocity' ? 'velocity-diagram' : ''}`}/>{features.map((feature) => { const chosen = myVotes[feature.id]; const selected = chosen === productId; const locked = Boolean(chosen && !selected); const count = totals[feature.id][productId]; return <button key={feature.id} onClick={() => vote(feature.id, productId)} disabled={busy === feature.id || Boolean(chosen)} aria-label={`${selected ? 'Your saved vote' : locked ? 'Voting locked' : `Vote for ${product.name}`} on ${feature.name}. ${count} votes.`} className={`section-hit section-${feature.id} ${selected ? 'selected' : ''} ${locked ? 'locked' : ''}`}><span className="section-content"><strong>{feature.name}</strong><span className="section-score"><span className="idle-count">{count}</span><ThumbsUp className="hover-thumb" size={16}/></span></span></button>; })}</div></article>; })}</div>
