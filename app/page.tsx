...
export default function Home() {
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <section className="mx-auto max-w-[1480px] px-5 pb-16 pt-8 sm:px-8">
      ...
      <div className="grid gap-4 lg:grid-cols-2">{(Object.keys(products) as ProductId[]).map((productId) => { const product = products[productId]; return <article key={productId} className={`product-card product-${productId} overflow-hidden rounded-[28px] border border-white/10 bg-[#111113] shadow-2xl shadow-black/20`}><div className="px-5 pt-6 text-center sm:px-7"><h2 className="text-2xl font-semibold uppercase tracking-[.12em]">{product.name}</h2></div><div className="shoe-map relative mt-1 aspect-[3/2] overflow-hidden"><img src={product.image} alt={`${product.name} sectioned running shoe diagram`} className={`pointer-events-none h-full w-full object-contain opacity-80 ${productId === 'velocity' ? 'velocity-diagram' : ''}`}/>{features.map((feature) => { const selected = myVotes[feature.id] === productId; const count = totals[feature.id][productId]; return <button key={feature.id} onClick={() => vote(feature.id, productId)} disabled={busy === feature.id} aria-label={`Vote for ${product.name} on ${feature.name}. ${count} votes.`} className={`section-hit section-${feature.id} ${selected ? 'selected' : ''}`}><span className="section-content"><strong>{feature.name}</strong><span className="section-score"><span className="idle-count">{count}</span><ThumbsUp className="hover-thumb" size={16}/></span></span></button>; })}</div></article>; })}</div>
