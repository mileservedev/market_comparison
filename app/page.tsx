...
const emptyTotals: VoteTotals = {
  comfort: { stride: 0, velocity: 0 },
  fit: { stride: 0, velocity: 0 },
  style: { stride: 0, velocity: 0 },
  color: { stride: 0, velocity: 0 },
  pricing: { stride: 0, velocity: 0 },
  availability: { stride: 0, velocity: 0 },
};
...
export default function Home() {
  ...
  const refresh = useCallback(async (fp?: string) => {
    try {
      const res = await fetch(
        `/api/votes${fp ? `?fingerprint=${encodeURIComponent(fp)}` : ''}`,
        { cache: 'no-store' },
      );
  ...
  const totalVotes = useMemo(
    () =>
      Object.values(totals).reduce((sum, t) => sum + t.stride + t.velocity, 0),
    [totals],
  );
  const productVotes = useMemo(
    () =>
      Object.values(totals).reduce(
        (sum, votes) => ({
          stride: sum.stride + votes.stride,
          velocity: sum.velocity + votes.velocity,
        }),
        { stride: 0, velocity: 0 },
      ),
    [totals],
  );
  const leader: ProductId | 'tie' | null =
    totalVotes === 0
      ? null
      : productVotes.stride === productVotes.velocity
        ? 'tie'
        : productVotes.stride > productVotes.velocity
          ? 'stride'
          : 'velocity';
  ...
  async function vote(feature: FeatureId, product: ProductId) {
    ...
    try {
      ...
      setMyVotes(data.myVotes);
      setMessage(
        `Your ${features.find((f) => f.id === feature)?.name} vote is counted.`,
      );
    } catch {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="mx-auto max-w-[1480px] px-3 pb-12 pt-6 sm:px-8 sm:pb-16 sm:pt-8">
        <div className="mb-7 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-[.22em] text-amber-400">
              Running shoe showdown
            </p>
            <h1 className="max-w-3xl text-3xl font-semibold tracking-tight sm:text-5xl">
              Every feature. One clear winner.
            </h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-400 sm:text-base">
              Tap a feature on either shoe to cast your preference. Your first
              choice for each feature is final.
            </p>
          </div>
        ...
        <div
          className={`leader-banner mb-4 ${leader === 'velocity' ? 'nike-leading' : ''}`}
          role="status"
          aria-live="polite"
        >
          ...
          <div>
            {leader === null ? (
              ...
              <>
                ...
                <p className="leader-name">adidas and Nike are tied</p>
              </>
            ) : (
              <>
                <p className="leader-eyebrow">Current overall winner</p>
                <p className="leader-name">
                  {products[leader].name} is leading
                </p>
        ...
        <div className="comparison-grid grid min-w-0 gap-4 lg:grid-cols-2">
          {(Object.keys(products) as ProductId[]).map((productId) => {
            ...
            const winning = leader === productId;
            return (
              <article
                key={productId}
                className={`product-card product-${productId} ${winning ? 'overall-winner' : ''} min-w-0 overflow-hidden rounded-[22px] border border-white/10 bg-[#111113] shadow-2xl shadow-black/20 sm:rounded-[28px]`}
              >
                ...
                <div className="shoe-map relative mt-1 aspect-[3/2] w-full overflow-hidden">
                  <img
                    src={product.image}
                    alt={`${product.name} sectioned running shoe diagram`}
                    className={`pointer-events-none h-full w-full object-contain opacity-80 ${productId === 'velocity' ? 'velocity-diagram' : ''}`}
                  />
                  {features.map((feature) => {
                    const chosen = myVotes[feature.id];
                    const selected = chosen === productId;
                    const locked = Boolean(chosen && !selected);
                    const count = totals[feature.id][productId];
                    return (
                      <button
                        key={feature.id}
                        onClick={() => vote(feature.id, productId)}
        ...
        <section className="mt-5 min-w-0 rounded-[22px] border border-white/10 bg-[#111113] p-4 sm:rounded-[28px] sm:p-7">
          ...
          <div className="grid gap-x-8 gap-y-6 md:grid-cols-2">
            {features.map((feature) => {
              ...
              const sum = pair.stride + pair.velocity;
              const left = sum ? Math.round((pair.stride / sum) * 100) : 50;
        ...
        <footer className="mt-5 flex min-w-0 flex-col justify-between gap-3 rounded-2xl border border-white/8 bg-white/[.025] px-4 py-4 text-xs leading-5 text-zinc-500 sm:flex-row sm:items-center sm:px-5">
          <p className="flex min-w-0 items-start gap-2">
            ...
            <span>
              To prevent duplicate votes, this demo stores your IP address,
